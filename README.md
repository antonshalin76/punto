...
- **Layout snapshot** — читается только подтверждённым X11 probe; при исправлении
  daemon отправляет настроенную комбинацию `hotkey` и продолжает только после
  подтверждения новой группы рабочим столом
...
- Chromium может сохранять PRIMARY после снятия видимого выделения.
  ...
  PRIMARY не дают такого разрешения. Ошибка X11-запроса, отмена или
  неподтверждённый контекст отзывают разрешение; оно сохраняется только если
  desktop принял полную комбинацию смены раскладки, но не переключился до
  общего deadline при неизменном наблюдаемом контексте. Поэтому старое
  выделение из другого поля того же приложения всё ещё может блокировать
  исправление слова. Receipt нового успешного исправления имеет отдельный
...
Настройте реальное переключение раскладки средствами GNOME/KDE/X11 и укажите
ту же комбинацию в `hotkey.modifier` и `hotkey.key`. При исправлении daemon
отправляет эту комбинацию как обычный ввод и ждёт, пока desktop manager
подтвердит требуемую XKB-группу. Punto не меняет системную настройку через
`gsettings`/`setxkbmap`; несовпадающая или неработающая комбинация приводит к
отказу начального preflight до удаления, выделения или вставки текста самим
Punto. Если desktop перестал обрабатывать комбинацию уже после отправленного
редактору действия, результат помечается как частичный: Punto не выполняет
слепой undo и не повторяет переключение. Незахваченная desktop-комбинация может
сама попасть в приложение согласно его обычной обработке клавиш.
...
Нужны US первой и RU второй группой. Проверьте, что комбинация обычного ручного
переключения настроена средствами рабочего стола и в точности совпадает с
`hotkey` в YAML. Punto использует эту комбинацию, но не меняет системную карту
и не создаёт desktop binding самостоятельно.
...
- Punto больше не изменяет XKB-группу напрямую. Для исправления он отправляет
  настроенную комбинацию `hotkey` и продолжает только после подтверждения новой
  группы рабочим столом. Если preflight не подтверждён, текст и clipboard не
  изменяются.
- Разделены lifecycle предыдущего Chromium receipt и receipt текущей успешной
  вставки. Безопасный timeout до изменения текста допускает повтор, а смена
  focus, pointer, locks, X11-сеанса или ошибка transport отзывает разрешение.
- Исправлена гонка подтверждения clipboard при плотном потоке X11-событий без
  увеличения общего 300-миллисекундного бюджета операции.
- Добавлены проверки четырёх последовательных ручных исправлений, повторных
  автоматических исправлений, mixed-layout history, desktop-shortcut lifecycle,
  post-dispatch deadline/context, GTK/VTE recovery и сохранения clipboard.
...
- Опрос статуса из трея больше не прерывает исправление: `GET_STATUS` и `STATS`
  ожидают завершения макроса; управляющие команды сохраняют отмену операции.
- Исправлена потеря пробела при задержке исходного отпускания клавиши. Перед
  изменением текста Punto проверяет отпускание клавиш в X-сервере; при долгом
  удержании отклоняет операцию в прежнем бюджете, не отпуская клавиши за пользователя.
...
- Добавлены девять E2E-сценариев в настоящем Chrome/Chromium: повторные ручные
  и автоматические исправления, защита нового выделения и сброс разрешения.
  Проверяются задержки настройки clipboard и отпускания клавиши, отмена и
  восстановление после отказа. Три дополнительных теста проверяют завершение
  браузерных процессов. Тесты запускаются
