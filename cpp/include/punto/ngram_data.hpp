...
// clang-format off
inline constexpr std::array<BigramEntry, 256> kEnBigrams = {{
    ...
    // === Дополнительные биграммы (расширение до 256) ===
    
    // Частые сочетания с гласными
    {'a', 'u', 35},  // au (cause, authority)
    {'o', 'a', 32},  // oa (road, coat)
    {'e', 'o', 30},  // eo (people)
    {'i', 'r', 28},  // ir (first, girl)
    {'u', 'a', 26},  // ua (quality, equal)
    {'a', 'g', 24},  // ag (page, manage)
    {'e', 'g', 22},  // eg (begin, legal)
    {'i', 'm', 20},  // im (time, important)
    {'o', 'm', 18},  // om (from, come)
    {'u', 'm', 16},  // um (number, column)
    
    // Частые сочетания с согласными
    {'n', 'c', 35},  // nc (once, since)
    {'r', 't', 32},  // rt (part, start)
    {'l', 'd', 30},  // ld (old, world)
    {'r', 'd', 28},  // rd (word, order)
    {'m', 'p', 26},  // mp (company, important)
    {'n', 'k', 24},  // nk (think, bank)
    {'r', 'n', 22},  // rn (return, turn)
    {'l', 't', 20},  // lt (result, felt)
    {'r', 'c', 18},  // rc (force, source)
    {'n', 'f', 16},  // nf (information)
    
    // Начала слов
    {'q', 'u', 40},  // qu (question, quality)
    {'e', 'x', 35},  // ex (example, experience)
    {'u', 'n', 32},  // un (under, until)
    {'d', 'i', 30},  // di (different, direction)
    {'m', 'i', 28},  // mi (might, million)
    {'p', 'u', 26},  // pu (public, put)
    {'s', 'u', 24},  // su (such, support)
    {'r', 'u', 22},  // ru (run, rule)
    {'b', 'o', 20},  // bo (both, body)
    {'g', 'u', 18},  // gu (government, guard)
    ...
    {'l', 'f', 32},  // lf (self, half)
    {'s', 'k', 30},  // sk (ask, task)
    {'n', 'l', 28},  // nl (only, mainly)
    {'e', 'n', 26},  // en (often, even) - дубль с весом
    {'r', 'l', 24},  // rl (world, girl)
    {'f', 'f', 22},  // ff (off, staff)
    {'s', 's', 20},  // ss (class, less)
    {'r', 'g', 18},  // rg (large, charge)
    
    // Программирование и IT контекст
    {'t', 'c', 40},  // tc (etc, match)
    {'f', 'u', 38},  // fu (function, full)
    {'m', 's', 35},  // ms (terms, systems)
    {'n', 'u', 32},  // nu (number, menu)
    {'d', 'a', 30},  // da (data, today)
    {'c', 'k', 28},  // ck (back, check)
    {'g', 't', 26},  // gt (get, right)
    {'p', 't', 24},  // pt (script, option)
    {'x', 't', 22},  // xt (text, next)
    {'b', 'a', 20},  // ba (base, back)
    
    // Дополнительные частые
    {'a', 'c', 36},  // ac (back, place)
    {'e', 'p', 34},  // ep (step, keep)
    {'i', 'v', 32},  // iv (give, live)
    {'o', 'k', 30},  // ok (book, look)
    {'u', 'c', 28},  // uc (much, such)
    {'a', 'w', 26},  // aw (law, away)
    {'e', 'f', 24},  // ef (before, left)
    {'i', 'x', 22},  // ix (six, fix)
    {'o', 'g', 20},  // og (program, log)
    {'u', 'd', 18},  // ud (could, would)
    
    // Финальные записи
    {'a', 'p', 35},  // ap (perhaps, appear)
    {'e', 'b', 32},  // eb (web, website)
    {'i', 'b', 30},  // ib (possible, library)
    {'o', 'b', 28},  // ob (job, problem)
    {'u', 'g', 26},  // ug (bug, drug)
    {'y', 'e', 24},  // ye (yes, yet)
    {'y', 's', 22},  // ys (system, always)
    {'y', 't', 20},  // yt (anything)
    {'z', 'e', 18},  // ze (size, realize)
    {'z', 'a', 16},  // za (organization)
    
    // Ещё частые биграммы
    {'h', 'r', 34},  // hr (through, three)
    {'k', 'i', 32},  // ki (kind, making)
    {'l', 's', 30},  // ls (also, else)
    {'m', 'b', 28},  // mb (number, member)
    {'n', 'i', 26},  // ni (night, opinion)
    {'p', 'i', 24},  // pi (opinion, picture)
    {'r', 'k', 22},  // rk (work, market)
    {'s', 'y', 20},  // sy (system, easy)
    {'t', 'u', 18},  // tu (actually, situation)
    {'v', 'i', 16},  // vi (view, service)
    
    // Дополнительные для достижения 256
    {'w', 'i', 34},  // wi (with, will)
    {'w', 'n', 32},  // wn (own, down)
    {'x', 'i', 30},  // xi (maximum, existing)
    {'x', 'p', 28},  // xp (experience, expect)
    {'y', 'i', 26},  // yi (anything, playing)
    {'y', 'n', 24},  // yn (dynamic, synonym)
    {'b', 'y', 22},  // by (by, maybe)
    {'d', 'y', 20},  // dy (body, study)
    {'f', 'y', 18},  // fy (verify, modify)
    {'g', 'y', 16},  // gy (energy, strategy)
    
    // Последние записи
    {'h', 'y', 32},  // hy (why, hypothesis)
    {'j', 'o', 30},  // jo (job, join)
    {'k', 'a', 28},  // ka (make, break)
    {'l', 'k', 26},  // lk (talk, walk)
    {'m', 'y', 24},  // my (my, many)
    {'n', 'a', 22},  // na (national, name)
    {'p', 's', 20},  // ps (perhaps, steps)
    {'r', 'm', 18},  // rm (form, term)
    {'s', 'c', 16},  // sc (describe, science)
    {'t', 'w', 14},  // tw (two, between)
}};
// clang-format on
...
inline constexpr std::array<BigramEntry, 256> kRuBigrams = {{
    ...
    // === Дополнительные русские биграммы (расширение до 256) ===
    ...
    // Частые биграммы (дополнительные)
    {'e', 'f', 36}, // уа
    {'f', 'e', 34}, // ау
    {'t', 'b', 32}, // еи
    {'b', 't', 30}, // ие
    ...
    // Программирование контекст (транслит)
    {'g', 'l', 35}, // пд
    {'c', 'h', 32}, // ср
    {'g', 'f', 30}, // па
    {'k', 'f', 28}, // ла
...
[[nodiscard]] inline std::uint8_t lookup_en_bigram(char first,
  ...
  return asm_utils::avx2_find_bigram(kEnBigrams.data(), kEnBigrams.size(),
                                     first, second);
...
[[nodiscard]] inline std::uint8_t lookup_ru_bigram(char first,
                                                   char second) noexcept {
#if defined(__x86_64__) && !defined(PUNTO_NO_ASM) && defined(__AVX2__)
  return asm_utils::avx2_find_bigram(kRuBigrams.data(), kRuBigrams.size(),
                                     first, second);
#elif defined(__x86_64__) && !defined(PUNTO_NO_ASM)
