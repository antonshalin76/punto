...
> **Безопасный режим v2.8.8.** Анализ, passthrough, CLI, tray и runtime-
...
- **Визуальный статус** — показывает, доступно ли изменение текста; в v2.8.8
  всегда `disabled`
...
- **Настройки...** — диалог (GTK3): параметры анализа и совместимая настройка
  ...
  - **max_rollback_words** сохраняется для совместимости конфига, но откат и
    повторный ввод в v2.8.8 не выполняются
...
| Режим                       | Действие                                                  |
| --------------------------- | --------------------------------------------------------- |
| **АВТО** (при пробеле/табе) | Анализирует слово; в v2.8.8 не изменяет текст или раскладку |
...
- **Строгий порядок телеметрии**: результаты учитываются по `task_id`, но в
  v2.8.8 ни один результат не применяется к документу.
...
поведение. В v2.8.8 результаты не изменяют документ.
...
| Комбинация           | Действие                                                                              |
| -------------------- | ------------------------------------------------------------------------------------- |
| **Pause**            | Зарезервировано; в v2.8.8 событие поглощается без изменения текста                    |
| **Shift+Pause**      | Зарезервировано; в v2.8.8 событие поглощается без изменения selection/clipboard       |
| **Ctrl+Pause**       | Зарезервировано; в v2.8.8 событие поглощается без изменения текста                    |
| **Alt+Pause**        | Зарезервировано; в v2.8.8 событие поглощается без изменения selection/clipboard       |
| **LCtrl+LAlt+Pause** | Зарезервировано; в v2.8.8 событие поглощается без изменения selection/clipboard       |
...
- **Layout snapshot** — читается только подтверждённым X11 probe; конфигурационный
  `hotkey` в v2.8.8 оставлен исключительно для совместимости схемы и игнорируется
...
- Для CI используйте `./build-deb.sh --non-interactive --skip-runtime-installs`.
  ...
  `SOURCE_DATE_EPOCH` применяется каноническая дата релиза v2.8.8
...
- Полной поддержки Wayland пока нет. Без подтверждённого X11 probe layout-
  ...
  телеметрии анализа, потому что изменение документа в v2.8.8 отключено.
...
- При logout или невалидном конфиге нового desktop-пользователя сохраняется
  последний успешно проверенный config snapshot. В v2.8.8 он влияет только на
...
- Безопасная замена слова и преобразование выделения требуют проверяемой,
  ...
  у универсального X11 clipboard, поэтому в v2.8.8 эти операции во всех окнах
...
- Clipboard backend ограничен payload 4 KiB и поддерживает только проверяемые
  text-targets без INCR. В v2.8.8 product entry points не вызывают его для
...
изменяются product runtime v2.8.8. Компонент сохранён только как изолированный
...
sudo dpkg -i "punto-switcher_2.8.8_$(dpkg --print-architecture).deb"
```
...
cd punto
./build-deb.sh
sudo dpkg -i punto-switcher_2.8.8_amd64.deb
```
...
```yaml
# Совместимость схемы: v2.8.8 принимает, сохраняет и игнорирует это поле
hotkey:
...
# v2.8.8 не выполняет эмуляцию исправляющих клавиш.
# Анализ слова при нажатии пробела/таба; в v2.8.8 без изменения текста
auto_switch:
  ...
  max_rollback_words: 5 # Совместимость; в v2.8.8 rollback не выполняется

# Совместимость: в безопасном режиме v2.8.8 звук исправлений не воспроизводится
sound:
  enabled: true
...
`hotkey.modifier` и `hotkey.key` в v2.8.8 оставлены только для обратной
...
Это ожидаемое поведение безопасного режима v2.8.8: комбинация поглощается до
чтения или изменения X11 selection и до отправки paste.

### Автопереключение не срабатывает

Изменение документа ожидаемо отключено в v2.8.8. Для проверки только анализа:
...
Punto v2.8.8 сам не переключает раскладку. Настройте комбинацию средствами
