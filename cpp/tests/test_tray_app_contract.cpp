...
struct TrayAppTestAccess {
  static void set_status_provider(TrayApp &app,
    ...
    app.status_provider_ = [provider = std::move(provider)] {
      ...
      return IpcClientResult{IpcClientError::None,
                             status,
                             {},
  ...
  static void set_status_setter(TrayApp &app,
  ...
  static void
  set_config_reloader(TrayApp &app,
                      std::function<bool(const std::string &)> reload) {
    app.config_reloader_ = std::move(reload);
  }

  static bool command_failed(TrayApp &app) { return app.last_command_failed_; }

  static void open_about(TrayApp &app) { app.on_about_clicked(nullptr, &app); }
};
...
struct PrivateSoundConfig {
  ...
  PrivateSoundConfig() {
    ...
    if (const char *value = std::getenv("HOME"))
      previous_home = value;
    expect(::setenv("HOME", root.c_str(), 1) == 0,
           "private config home installed");
    path = punto::SettingsDialog::get_user_config_path();
    std::filesystem::create_directories(path.parent_path());
  }
  ~PrivateSoundConfig() {
    if (previous_home)
      (void)::setenv("HOME", previous_home->c_str(), 1);
    else
      (void)::unsetenv("HOME");
...
void exercise_sound_menu(punto::TrayApp &app, SoundDialogAction &action) {
  GtkWidget *item = action.sound_only
                        ? punto::TrayAppTestAccess::sound_item(app)
                        : punto::TrayAppTestAccess::settings_item(app);
  expect(item != nullptr, "requested preferences item exists");
  expect(gtk_widget_get_sensitive(item),
         "sound preferences are available from tray");
  const guint error_closer = g_timeout_add(
      5,
      [](gpointer data) -> gboolean {
        ...
        for (GList *window = windows; window; window = window->next) {
          if (GTK_IS_MESSAGE_DIALOG(window->data)) {
            ...
            gchar *text = nullptr;
            g_object_get(window->data, "message-type", &type, "text", &text,
                         nullptr);
            state.error_seen =
                type == GTK_MESSAGE_ERROR && text &&
                std::string_view{text}.find(
                    "Не удалось подтвердить сохранение настроек") !=
      ...
      &action);
  const guint driver = g_idle_add(
      [](gpointer data) -> gboolean {
        auto &state = *static_cast<SoundDialogAction *>(data);
        GList *windows = gtk_window_list_toplevels();
        GtkWidget *dialog = nullptr;
        for (GList *window = windows; window; window = window->next) {
          if (GTK_IS_DIALOG(window->data) &&
              !GTK_IS_MESSAGE_DIALOG(window->data)) {
        ...
        try {
          ...
          const unsigned int checks =
              count_widgets(dialog, GTK_TYPE_CHECK_BUTTON);
          const unsigned int spins =
              count_widgets(dialog, GTK_TYPE_SPIN_BUTTON);
          const unsigned int notebooks =
              count_widgets(dialog, GTK_TYPE_NOTEBOOK);
          state.correct_view =
              title &&
              (state.sound_only
                   ? std::string_view{title} ==
                             "Звук исправлений Punto Switcher" &&
          ...
          if (sound) {
            state.initial_enabled =
                gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(sound));
            if (state.toggle) {
            ...
            if (state.reenter_item) {
              ...
              for (GList *window = open_windows; window;
                   window = window->next) {
                if (GTK_IS_DIALOG(window->data))
                  ++dialogs;
              }
              g_list_free(open_windows);
              state.reentry_preserved =
                  dialogs == 1 &&
                  original_title == gtk_window_get_title(GTK_WINDOW(dialog)) &&
                  (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(sound)) !=
                   FALSE) == (state.toggle ? !state.initial_enabled
                                           : state.initial_enabled);
            }
            state.save_enabled =
                gtk_widget_get_sensitive(gtk_dialog_get_widget_for_response(
                    GTK_DIALOG(dialog), GTK_RESPONSE_ACCEPT));
            if (!state.fail_path.empty()) {
              std::filesystem::rename(state.fail_path,
                                      state.fail_path.string() + ".saved");
  ...
  if (g_main_context_find_source_by_id(nullptr, driver))
    g_source_remove(driver);
  ...
  expect(action.correct_view,
         action.sound_only
             ? "sound menu opens dedicated sound-only controls, not general "
               "Settings"
...
void test_sound_menu_uses_settings_persistence_and_failure_readback() {
  ...
  punto::TrayAppTestAccess::set_config_reloader(
      app, [&](const std::string &path) {
        reload_path = path;
        ++reloads;
        return false; // No host IPC: model an unavailable service after a saved
                      // preference.
      });
  ...
  expect(loaded.result == punto::ConfigResult::Ok &&
             punto::serialize_config(loaded.config) ==
                 punto::serialize_config(expected),
  ...
  expect(punto::TrayAppTestAccess::command_failed(app),
         "a saved preference with an unconfirmed reload is visible as a tray failure");
  ...
  expect(failure.error_seen && reloads == 1,
         "failed save reports error without reload");
  ...
  expect(!reopen.initial_enabled && !reopen.save_enabled && reloads == 1 &&
             !reopen.error_seen,
  ...
  expect(punto::serialize_config(loaded.config) ==
             punto::serialize_config(expected),
...
void test_status_poll_is_single_flight_and_does_not_block_main_context() {
  ...
  expect(!app->shutdown_background(20ms),
         "stalled status worker is reported to the process owner");
  expect(std::chrono::steady_clock::now() - shutdown_started < 100ms,
         "tray shutdown honors the caller deadline");
  ...
  expect(finished.load(std::memory_order_acquire), "status poll completes");
  expect(app->shutdown_background(500ms),
         "completed worker and queued callback drain before teardown");
  app.reset();
}
...
void test_periodic_read_does_not_disable_confirmed_toggle() {
  punto::TrayApp app;
  GtkWidget *toggle = punto::TrayAppTestAccess::create_menu(app);
  punto::TrayAppTestAccess::set_status_provider(
      app, [] { return punto::ServiceStatus::Enabled; });
  punto::TrayAppTestAccess::request_status_update(app);
  wait_for([&] { return gtk_widget_get_sensitive(toggle) != FALSE; },
           "initial status enables toggle");
  ...
  punto::TrayAppTestAccess::set_status_provider(app, [&] {
    ...
    return punto::ServiceStatus::Enabled;
  });
  punto::TrayAppTestAccess::request_status_update(app);
  wait_for([&] { return started.load(std::memory_order_acquire); },
           "periodic read starts");
  expect(gtk_widget_get_sensitive(toggle) != FALSE,
         "background status read keeps the confirmed control available");
  release.store(true, std::memory_order_release);
  pump_for(10ms);
}
...
void test_capability_is_projected_separately_from_auto_status() {
  ...
  for (const auto &test :
       {Case{Capability::Disabled, Status::Disabled,
        ...
        Case{Capability::X11, Status::Disabled, "Автокоррекция слов", true},
        Case{Capability::X11, Status::Enabled, "Автокоррекция слов", true},
        Case{Capability::Unknown, Status::Unknown,
             "Автокоррекция: состояние неизвестно", false}}) {
    punto::TrayApp app;
    GtkWidget *toggle = punto::TrayAppTestAccess::create_menu(app);
    std::atomic<unsigned int> calls{0};
    punto::TrayAppTestAccess::set_status_setter(app, [&](bool) {
      ++calls;
      return true;
    });
    punto::TrayAppTestAccess::set_snapshot_provider(app, [test] {
      return punto::IpcClientResult{
          punto::IpcClientError::None, test.status, {}, test.capability};
    });
    punto::TrayAppTestAccess::request_status_update(app);
    wait_for(
        [&] {
          return std::string_view{gtk_menu_item_get_label(
                     GTK_MENU_ITEM(toggle))} == test.label &&
    ...
    expect((gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(toggle)) !=
            FALSE) == (test.status == Status::Enabled),
           "checkbox follows runtime automation");
    expect(gtk_widget_get_sensitive(
               punto::TrayAppTestAccess::sound_item(app)) != FALSE,
           "sound preference editor stays available independently of runtime "
           "capability");
    if (!test.sensitive) {
      gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(toggle), TRUE);
      pump_for(5ms);
      expect(calls.load() == 0,
             "legacy and unknown capability cannot submit SET");
    }
  }
}
...
void test_toggle_uses_readback_even_after_rejected_command() {
  for (int scenario = 0; scenario < 4; ++scenario) {
    ...
    punto::TrayAppTestAccess::set_snapshot_provider(app, [=] {
      ...
      return punto::IpcClientResult{punto::IpcClientError::None,
                                    enabled->load()
                                        ? punto::ServiceStatus::Enabled
                                        : punto::ServiceStatus::Disabled,
                                    {},
    ...
    punto::TrayAppTestAccess::request_status_update(app);
    wait_for([&] { return gtk_widget_get_sensitive(toggle) != FALSE; },
             "initial modern snapshot");
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(toggle), TRUE);
    expect(gtk_widget_get_sensitive(toggle) == FALSE,
           "toggle is disabled while command/readback is pending");
    expect(gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(toggle)) == FALSE,
           "toggle does not present optimistic success");
    wait_for(
        [&] {
          return scenario == 3
                     ? punto::TrayAppTestAccess::current_status(app) ==
                           punto::ServiceStatus::Unknown
                     : gtk_widget_get_sensitive(toggle) != FALSE;
        },
        "command result is reconciled through authoritative snapshot");
    expect(setter_calls->load() == 1 && last_requested->load(),
           "one enable command was submitted");
    const bool expected_checked = scenario == 0 || scenario == 2;
    expect((gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(toggle)) !=
            FALSE) == expected_checked,
           "checkbox shows readback including nondurable ERROR with visible "
           "commit");
    expect(punto::TrayAppTestAccess::command_failed(app) ==
    ...
    if (scenario == 1 || scenario == 2) {
      ...
      const bool feedback =
          tooltip != nullptr &&
          std::string_view{tooltip}.find("Команда не подтверждена") !=
              std::string_view::npos;
      g_free(tooltip);
...
void test_about_describes_limited_word_capability() {
  ...
  for (GList *window = windows; window != nullptr; window = window->next) {
    ...
    for (GList *child = children; child != nullptr; child = child->next) {
      if (GTK_IS_LABEL(child->data)) {
        ...
        correct =
            text.find("Исправление слов и выделенного текста в X11") !=
                std::string_view::npos &&
  ...
  g_list_free(windows);
  expect(correct,
         "About describes restored X11 word and selection corrections");
}

void test_invalid_config_is_reported_without_showing_default_settings() {
  PrivateSoundConfig fixture;
  {
    std::ofstream output{fixture.path};
    output << "unknown_product_key: true\n";
  }
  punto::TrayApp app;
  punto::TrayAppTestAccess::create_menu(app);
  unsigned int reloads = 0;
  ...
  bool error_seen = false;
  const guint closer = g_timeout_add(
      5,
      [](gpointer data) -> gboolean {
        ...
        for (GList *window = windows; window; window = window->next) {
          ...
          gchar *text = nullptr;
          g_object_get(window->data, "text", &text, nullptr);
          seen = text && std::string_view{text}.find(
  ...
  gtk_menu_item_activate(GTK_MENU_ITEM(
      punto::TrayAppTestAccess::settings_item(app)));
  if (g_main_context_find_source_by_id(nullptr, closer)) {
    g_source_remove(closer);
  }
  expect(error_seen && reloads == 0,
         "invalid config is reported without a default-valued edit or reload");
}

[[gnu::noinline]] void inject_owned_leak_for_sanitizer_negative_control() {
  // Explicit opt-in proves the external Fontconfig exception cannot hide our
  // leaks.
...
int main(int argc, char **argv) {
  try {
    const bool negative_control =
        argc == 2 && std::string_view{argv[1]} == "--lsan-negative-control";
    ...
    test_periodic_read_does_not_disable_confirmed_toggle();
    test_capability_is_projected_separately_from_auto_status();
    test_toggle_uses_readback_even_after_rejected_command();
    test_invalid_config_is_reported_without_showing_default_settings();
