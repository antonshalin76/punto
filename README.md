...
- Для CI используйте `./build-deb.sh --non-interactive --skip-runtime-installs`.
  ...
  `SOURCE_DATE_EPOCH` применяется каноническая дата релиза v2.8.9
...
sudo dpkg -i "punto-switcher_2.8.9_$(dpkg --print-architecture).deb"
```
...
cd punto
./build-deb.sh
sudo dpkg -i punto-switcher_2.8.9_amd64.deb
```
